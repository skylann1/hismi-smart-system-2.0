...
const dummyKegiatanData: { [key: string]: KegiatanFormData } = {
  "keg-01": {
...
export default function EditKegiatanPage() {
  ...
  useEffect(() => {
    if (id && dummyKegiatanData[id]) {
      setFormData(dummyKegiatanData[id]);
    }
  }, [id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isFormValid) {
      console.log("Data kegiatan yang akan diupdate:", formData);
    ...
    <DashboardSection className="w-full min-h-full p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-6">
      ...
      <div className="flex flex-col gap-4 items-center lg:items-start text-center lg:text-left">
        ...
        <span className="text-sm font-[400] text-gray-500 leading-relaxed">
          Ubah detail kegiatan. Pastikan semua informasi seperti divisi
          penyelenggara, lokasi, dan tanggal pelaksanaan sudah benar.
        </span>
        <div className="w-full sm:w-3/4 mt-4 flex justify-center">
          <Image
            src="/assets/undraw/off-road.svg"
            height={5000}
            width={5000}
            alt="edit kegiatan illustration"
