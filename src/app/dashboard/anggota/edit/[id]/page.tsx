...
import InputTextSkeleton from "@/components/ui/skeleton/InputTextSkeleton";
import InputFileSkeleton from "@/components/ui/skeleton/InputFIleSkeleton";
...
export default function Page() {
  ...
  useEffect(() => {
    const getData = async () => {
      setLoadingData(true);
      try {
        const res = await fetch("/dashboard/api/anggota?id=" + id);
        ...
        setFormData({
          ...
          nim: user.data.nim,
          jenjang_pendidikan: user.data.jenjang_pendidikan,
          semester: user.data.semester,
          tipe_kelas: user.data.tipe_kelas,
          tahun_masuk: user.data.tahun_masuk,
          divisi: user.data.divisi,
          role: user.data.role,
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const imageError = validateImage(formData.imageUrl);
    ...
    try {
      ...
      const res = await fetch(`/dashboard/api/anggota/edit/${id}`, {
        method: "PATCH",
        body: formDataToSend,
      });
