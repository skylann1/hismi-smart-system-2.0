...
export default function RekapAbsenPage() {
    ...
    useEffect(() => {
        ...
        if ((session?.user as any)?.id) {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            fetch(`/dashboard/api/kehadiran/rekap?userId=${(session?.user as any).id}`)
                .then((res) => res.json())
                .then((json) => {
                    if (json.success) {
                        setData(json.data);
    ...
    if (loading) return <div className="p-6">Loading rekap absensi...</div>;

    return (
        <div className="p-6 w-full">
            <div className="mb-8">
                <h1 className="text-2xl font-bold text-gray-800">Rekap Absensi Saya</h1>
                <p className="text-gray-500">Berikut adalah riwayat kehadiran Anda di berbagai kegiatan, proker, dan pertemuan.</p>
            </div>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {data.map((item, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                        <td className="px-6 py-4 font-medium text-gray-900">{item.parentJudul}</td>
