...
const dummyProkerData: { [key: string]: ProkerFormData } = {
  "proker-01": {
    judul: "Kelas Belajar Bareng Web Development",
...
export default function EditProkerPage() {
  ...
  useEffect(() => {
    if (id && dummyProkerData[id]) {
      setFormData(dummyProkerData[id]);
    }
  }, [id]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (isFormValid) {
      console.log("Data proker yang akan diupdate:", formData);
    ...
    <DashboardSection className="w-full min-h-full p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-6">
      ...
      <div className="flex flex-col gap-4 items-center lg:items-start text-center lg:text-left">
        ...
        <span className="text-sm font-[400] text-gray-500 leading-relaxed">
          Ubah detail program kerja. Pastikan semua informasi seperti divisi
          penanggung jawab, lokasi, dan target tanggal sudah benar.
        </span>
        <div className="w-full sm:w-3/4 mt-4 flex justify-center">
          <Image
            src="/assets/undraw/project-team.svg"
            height={5000}
            width={5000}
            alt="edit proker illustration"
