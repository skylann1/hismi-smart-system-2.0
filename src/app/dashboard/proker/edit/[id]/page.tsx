...
export default function TambahProkerPage({
      ...
      const submitProker = async () => {
        try {
          ...
          dispatch(
            alertIsAktif({
              status: true,
              title: "Sukses! Proker berhasil diupdate.",
              message: "Proker telah berhasil diperbarui dalam sistem.",
            })
          );
          router.push("/dashboard/proker");
        } catch (error) {
  ...
  return (
    <DashboardSection className="w-full min-h-full p-4 sm:p-6 grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-6">
      ...
      {/* Right Section (Form) */}
      {isLoading ? (
        <div className="flex justify-center items-center w-full h-full bg-white rounded-lg p-4 sm:p-6 shadow-lg">
          <span className="text-gray-500">Memuat data proker...</span>
        </div>
      ) : (
        <form
